...
public class RemoveArchiveAction extends Action {
	 ...
	 * Used for localized.
	 */
	private static final Localized L = Localized
			.load(RemoveArchiveAction.class);
	...
	static {
		...
		imageRegistry.put(ICON_LIST_REMOVE_16, ImageDescriptor.createFromFile(
				RemoveAction.class, "images/list-remove-16.png"));//$NON-NLS-1$
	}
	...
	private void handleException(int operation, ManagerException e) {
		...
		String message = operation == REMOVE_ID ? L
				.get("RemoveArchiveAction.cantRemoveObject") //$NON-NLS-1$
		...
		if (e.isConstraintViolationException()) {
			shortDetail = L
					.get("RemoveArchiveAction.constraintViolationException"); //$NON-NLS-1$
		}

		StringWriter sw = new StringWriter();
		e.printStackTrace(new PrintWriter(sw));
		DetailMessageDialog.openDetailWarning(this.shellProvider.getShell(),
				L.get("RemoveArchiveAction.exception.title"), //$NON-NLS-1$
				message, shortDetail, sw.toString());
	}
	...
	public void run() {
		...
		String[] buttons = new String[] {
				L.get("RemoveArchiveAction.removeLabel"), //$NON-NLS-1$
				L.get("RemoveArchiveAction.archiveLabel"), //$NON-NLS-1$
				IDialogConstants.CANCEL_LABEL };

		// Prompt the user to confirm the remove or the archive
		DetailMessageDialog dlg = new DetailMessageDialog(
				this.shellProvider.getShell(),
				L.get("RemoveArchiveAction.confirmMessage.title"), //$NON-NLS-1$
				null, L.get("RemoveArchiveAction.confirmMessage.message"), //$NON-NLS-1$
				L.get("RemoveArchiveAction.confirmMessage.shortDetail"), //$NON-NLS-1$
				null, MessageDialog.QUESTION, buttons, 2, null, false);
	...
	public void setObjects(List<ManagedObject> objects) {
		firePropertyChange(
				OBJECTS,
				getObjects(),
				this.objects = (objects == null || objects.size() == 0 ? null
						: Collections
								.unmodifiableList(new ArrayList<ManagedObject>(
										objects))));
		setEnabled(canRun());
	}
