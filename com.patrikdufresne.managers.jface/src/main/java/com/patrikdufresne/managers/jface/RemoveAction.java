...
public class RemoveAction extends Action {
	...
	public void run() {
		...
		DetailMessageDialog dlg = new DetailMessageDialog(
				...
				L.get("RemoveAction.confirmRemove.title"), //$NON-NLS-1$
				null,
				this.message != null ? this.message : L
						.get("RemoveAction.confirmRemove.message"), //$NON-NLS-1$
				this.shortDetail != null ? this.shortDetail : L
						.get("RemoveAction.confirmRemove.shortDetail"), null, //$NON-NLS-1$
		...
		try {
			...
			DetailMessageDialog.openDetailWarning(
					this.shellProvider.getShell(),
					L.get("RemoveAction.confirmRemove.title"),
					L.get("RemoveAction.cantRemoveObject"), //$NON-NLS-1$
	...
	public void setObject(ManagedObject object) {
		firePropertyChange(OBJECT, getObject(),
				this.objects = (object == null ? null : Arrays.asList(object)));
		setEnabled(canRun());
	}
