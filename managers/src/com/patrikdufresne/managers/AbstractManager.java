...
public abstract class AbstractManager<T extends ManagedObject> implements
	...
	protected void checkObject(Collection<? extends T> s) {
		...
		Iterator<? extends T> iter = s.iterator();
		while (iter.hasNext()) {
			if (!objectClass().isInstance(iter.next()))
				throw new ClassCastException();
		}
	}
	...
	public T get(final int id) throws ManagerException {
		return getManagers().query(new Query<T>() {
			...
			public T run() throws ManagerException {
				return (T) ManagerContext.getDefaultSession().get(
						objectClass(), Integer.valueOf(id));
			}
		});
	}
	...
	public List<T> list() throws ManagerException {
		return getManagers().query(new Query<List<T>>() {
			...
			public List<T> run() throws ManagerException {
				return (List<T>) ManagerContext.getDefaultSession()
	...
	protected void postRemoveObjects(Collection<? extends T> s)
			throws ManagerException {
		// Implemented by subclasses
	}
	...
	protected void postUpdateObjects(Collection<? extends T> s)
			throws ManagerException {
		// Implemented by subclasses
	}
	...
	protected void preAddObjects(Collection<? extends T> s)
			throws ManagerException {
		// Implemented by sub-class
	}
	...
	protected void preRemoveObjects(Collection<? extends T> s)
			throws ManagerException {
		// Implemented by sub-class
	}
	...
	protected void preUpdateObjects(Collection<? extends T> t)
			throws ManagerException {
		// Implemented by sub-class
	}
	...
	public int size() throws ManagerException {
		return getManagers().query(new Query<Integer>() {
			...
			public Integer run() throws ManagerException {
				return (Integer) ManagerContext.getDefaultSession()
						.createCriteria(objectClass())
						.setProjection(Projections.count("id")).uniqueResult(); //$NON-NLS-1$
			}
		}).intValue();
	}
