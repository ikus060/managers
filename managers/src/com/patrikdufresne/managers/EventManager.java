...
public class EventManager {
	...
	public void hook(int eventType, Class<?> clazz, IManagerObserver listener) {
		...
		if (this.clazzs == null)
			this.clazzs = new Class<?>[GROW_SIZE];
		if (this.listeners == null)
			this.listeners = new IManagerObserver[GROW_SIZE];
		...
		if (index == length) {
			...
			IManagerObserver[] newListeners = new IManagerObserver[length
					+ GROW_SIZE];
	...
	public boolean hooks(int eventType, Class<?> clazz) {
		...
		for (int i = 0; i < this.types.length; i++) {
			if ((this.types[i] & eventType) != 0 && this.clazzs[i] == clazz)
				return true;
		}
		return false;
	}
	...
	private void remove(int index) {
		if (this.level == 0) {
			...
			System.arraycopy(this.types, index + 1, this.types, index, end
					- index);
			System.arraycopy(this.clazzs, index + 1, this.clazzs, index, end
					- index);
			System.arraycopy(this.listeners, index + 1, this.listeners, index,
					end - index);
			index = end;
		} else {
			if (this.level > 0)
	...
	public void sendEvent(ManagerEvent event) {
		...
		try {
			...
			// type
			for (int i = 0; i < this.types.length; i++) {
				if ((this.types[i] & event.type) != 0
						&& this.clazzs[i].isAssignableFrom(event.clazz)
	...
	public void sendEvents(EventTable table) {
		...
		for (int i = 0; i < table.types.size(); i++) {
			...
			while (index < events.size()
					&& (!events.get(index).clazz.equals(cls) || events
							.get(index).type != eventType)) {
				index++;
			}
	...
	public void unhook(int eventType, Class<?> clazz, IManagerObserver listener) {
		...
		if (this.types == null || this.clazzs == null || this.listeners == null)
			return;
		for (int i = 0; i < this.types.length; i++) {
			if (this.types[i] == eventType && this.clazzs[i] == clazz
