...
public class ManagerUpdateListStrategy extends UpdateListStrategy {
     ...
     */
    public static ManagerUpdateListStrategy create(Managers managers, boolean provideDefaults, int updatePolicy, final IObservableValue observableValue) {
        if (observableValue == null) {
            throw new IllegalArgumentException();
        }
        // Create an anonymous class overriding the findManagedObject method to
        // provide the value of the observable.
        return new ManagerUpdateListStrategy(managers, provideDefaults, updatePolicy) {
     ...
     * @return a new update set strategy
     */
    public static ManagerUpdateListStrategy create(Managers managers, int updatePolicy, final IObservableValue observableValue) {
        return create(managers, true, updatePolicy, observableValue);
    }
     ...
     * @return a new update set strategy
     */
    public static ManagerUpdateListStrategy create(Managers managers, final IObservableValue observableValue) {
        return create(managers, true, UpdateValueStrategy.POLICY_UPDATE, observableValue);
    }
     ...
     *            {@link ManagedObject}.
     */
    public ManagerUpdateListStrategy(Managers managers) {
     ...
     *            {@link #POLICY_CONVERT}, or {@link #POLICY_UPDATE}
     */
    public ManagerUpdateListStrategy(Managers managers, boolean provideDefaults, int updatePolicy) {
     ...
     *            {@link #POLICY_CONVERT}, or {@link #POLICY_UPDATE}
     */
    public ManagerUpdateListStrategy(Managers managers, int updatePolicy) {
    ...
    protected IStatus doAdd(IObservableList observableList, Object element, int index) {
        ...
        IStatus status = super.doAdd(observableList, element, index);
        if (!status.isOK()) {
            return status;
        }
        // Persists the modification
        return persist(observableList);
    }
    ...
    protected IStatus doMove(IObservableList observableList, int oldIndex, int newIndex) {
        // Call the super method to sets the value
        IStatus status = super.doMove(observableList, oldIndex, newIndex);
        if (!status.isOK()) {
            return status;
        }
        // Persists the modification
    ...
    protected IStatus doReplace(IObservableList observableList, int index, Object element) {
        // Call the super method to sets the value
        IStatus status = super.doReplace(observableList, index, element);
        if (!status.isOK()) {
            return status;
        }
        // Persists the modification
     ...
     *            {@link ManagedObject}
     */
    protected IStatus persist(IObservableList observable) {
        ...
        try {
            ...
            e.printStackTrace();
            return ValidationStatus.error("Failure to persist the ManagedObject", e); //$NON-NLS-1$
        }
        return ValidationStatus.ok();
    }
