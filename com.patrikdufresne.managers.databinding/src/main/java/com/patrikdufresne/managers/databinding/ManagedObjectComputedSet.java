...
public class ManagedObjectComputedSet extends AbstractObservableSet implements IManagerObservable {
     ...
     * 
     */
    private class PrivateInterface implements IChangeListener, IStaleListener, IManagerObserver {
        ...
        public void handleStale(StaleEvent event) {
            if (!ManagedObjectComputedSet.this.dirty) makeStale();
        }
    ...
    protected void startListening() {
        ...
        if (this.events != null) {
            for (Entry<Class, Integer> e : this.events.entrySet()) {
                getManagers().addObserver(e.getValue().intValue(), (Class) e.getKey(), this.privateInterface);
            }
        }
    }
    ...
    protected void stopListening() {
        ...
        if (this.events != null) {
            for (Entry<Class, Integer> e : this.events.entrySet()) {
                getManagers().removeObserver(e.getValue().intValue(), (Class) e.getKey(), this.privateInterface);
            }
        }
    }
